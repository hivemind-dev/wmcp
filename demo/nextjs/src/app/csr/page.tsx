...
export default function CSRPage() {
  ...
  useEffect(() => {
    ...
    const host = new WmcpHost(counter.wmcpClient, { baseUrl: "" });

    host.connect({
      "persist:load": {
        resolve: () => ({ method: "GET", path: "/api/counter" }),
      },
      "persist:save": {
        resolve: (p) => ({
          method: "POST",
  ...
  return (
    <div className="page-stack">
      <div>
        <h1 className="page-title">CSR Mode</h1>
        <p className="page-desc">
          Module owns counter logic. <strong>host:requires</strong> bound as{" "}
          <strong>HTTP adapters</strong>. Host calls{" "}
